...
- 上传文档时自动切分文本并写入 `document_chunks` 表，检索时直接基于文档片段匹配，提高 RAG 结构清晰度和扩展性
...
用户可以上传 `.txt`、`.md`、`.pdf` 文档，系统会解析文档内容并保存到 SQLite 数据库中。上传文档时，后端会自动将文档切分为多个文本片段，并保存到 `document_chunks` 表中。用户提问时，系统会根据问题从文档片段中检索相关内容，再将相关片段注入提示词，引导 DeepSeek 基于上传资料回答。

v2.3 相比之前版本，不再每次提问时临时切分全文，而是在上传阶段完成文档分块入库，检索时直接查询 `document_chunks` 表，更接近真实 RAG 系统的结构。
